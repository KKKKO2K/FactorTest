...
OUT = ROOT / "analysis" / "methodology_results"
...
def load_basic() -> tuple[pd.DataFrame, pd.DataFrame, dict[pd.Timestamp, pd.DataFrame]]:
    ...
    mcaps = pd.concat(cap_rows, ignore_index=True).pivot(index="date", columns="code", values="mcap").sort_index()
    return returns, mcaps, panels
...
def build_signal_panel(
    ...
    p["score_k200"] = 0.75 * p["value"] + 0.15 * p["rev_fy1"].fillna(0.5) + 0.10 * p["rev_12"].fillna(0.5)
    p["score_kq"] = 0.70 * p["rev_fy1"] + 0.30 * p["value"]
...
def select_names(
    ...
    eligible_all = u.index[required].tolist()
    entrants = u.loc[entry].sort_values("score", ascending=False).index.tolist()
    if not buffer:
        return entrants[:n_target], eligible_all
    ...
    return (keep + fill[: max(0, n_target - len(keep))]), eligible_all
...
def target_weights(
    ...
    if not names:
        return pd.Series(dtype=float)
    x = u.loc[names].copy().sort_values("score", ascending=False)
    n = len(x)
    if method == "equal":
...
def simulate_config(
    returns: pd.DataFrame,
    prepared: dict[str, dict[pd.Timestamp, pd.DataFrame]],
    strategy: str,
    interval: int,
    n_target: int,
    method: str,
    ...
    for dt in dates:
        ...
        records.append({
            ...
            "gross_ret": gross,
            "bench_ret": bench,
            "turnover_event": pending_turnover,
            "n_names": len(weights),
        })
        pending_turnover = 0.0
        ...
        if dt in rebalance_dates and dt in prepared[strategy]:
            ...
            turnover = turnover_to_target(weights, target)
            bench_target = pd.Series(1.0 / len(eligible), index=eligible) if eligible else pd.Series(dtype=float)
            bench_turnover = turnover_to_target(bench_w, bench_target)
            ...
            rebalances.append({
                ...
                "max_weight": float(target.max()) if not target.empty else np.nan,
                "hhi": float((target ** 2).sum()) if not target.empty else np.nan,
            })
            weights = target
            bench_w = bench_target
            current_names = names
...
def summarize_run(
    ...
    cost = d["turnover_event"] * cost_bps / 10000.0
    net = d["gross_ret"] - cost
    excess = net - d["bench_ret"]
    gross_m = metrics(d["gross_ret"])
    net_m = metrics(net)
    ex_m = metrics(excess)
...
def main():
    returns, mcaps, panels = load_basic()
    ...
    for j, dt in enumerate(base_dates, 1):
        if dt not in panels:
            continue
        full = build_signal_panel(dt, panels[dt], factor_cache, returns)
    ...
    for i, cfg in enumerate(configs, 1):
        daily, rebal = simulate_config(returns, prepared, **{
        ...
        for year, g in d.groupby(d.index.year):
            period_rows.append({
                **cfg,
                "year": int(year),
                "net_return": float((1 + g["net"]).prod() - 1),
                "bench_return": float((1 + g["bench_ret"]).prod() - 1),
                "excess_return": float((1 + g["excess"]).prod() - 1),
                "turnover": float(g["turnover_event"].sum()),
            })
        if i % 20 == 0:
    ...
    pd.DataFrame(period_rows).to_csv(OUT / "methodology_yearly_results.csv", index=False, encoding="utf-8-sig")
    ...
    merged = test30.merge(
        train30[keys + ["excess_cagr", "excess_sharpe", "net_cagr", "net_sharpe"]],
        on=keys, suffixes=("_test", "_train")
    )
    ...
    for strategy in test30["strategy"].unique():
        ...
        for dim in ("interval", "n_target", "weighting", "buffer", "cap_mult"):
            a = s.groupby([dim]).agg(
                ...
                median_excess_sharpe=("excess_sharpe", "median"),
                median_net_cagr=("net_cagr", "median"),
                median_turnover=("annual_turnover", "median"),
                n_configs=("excess_cagr", "size"),
            ).reset_index()
            a["strategy"] = strategy
            a["dimension"] = dim
            a = a.rename(columns={dim: "level"})
            agg_rows.append(a)
    pd.concat(agg_rows, ignore_index=True).to_csv(OUT / "parameter_medians.csv", index=False, encoding="utf-8-sig")

    lines = [
        "# Rebalance and weighting methodology backtest",
        "",
        f"- Data: {returns.index.min().date()} to {returns.index.max().date()}",
        f"- Train/test split: {SPLIT_DATE.date()}",
        f"- Configurations: {len(configs)}; cost cases: {COSTS_BPS}",
        "- Portfolio timing: signal at t close, target applies from t+1; daily weights drift between rebalances.",
        "- Turnover includes initial deployment and cash leg; cost charged on the next trading day.",
        "- Benchmark: equal-weight portfolio of factor-eligible names, refreshed on the same rebalance schedule.",
        "- Missing held-stock daily return is treated as 0. This needs a later delisting/halts audit.",
        "",
    ]
    for strategy in ("K200_VETO", "KOSDAQ_REV_FIRST"):
        lines += [f"## {strategy}: robust top configurations at 30bp", ""]
        top = robust[robust["strategy"] == strategy].head(10)
        cols = keys + [
            "excess_cagr_train", "excess_sharpe_train", "excess_cagr_test", "excess_sharpe_test",
            "net_cagr_test", "net_sharpe_test", "annual_turnover", "avg_names", "avg_max_weight",
        ]
